...
interface BaseDataEditFormProps {
  pedido: BasePedido;
  onSubmit: (id: number, updates: Partial<BasePedido>) => void;
  onClose: () => void;
}

const BaseDataEditForm: React.FC<BaseDataEditFormProps> = ({ 
  pedido, 
  onSubmit, 
  onClose 
}) => {
  const [formData, setFormData] = useState<BasePedido>(pedido);
  ...
  const handleInputChange = (field: keyof BasePedido, value: string) => {
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-800">
              Editar Pedido #{pedido.id} - Sistema Base
            </h2>
            <button
              onClick={onClose}
...
export default BaseDataEditForm;
